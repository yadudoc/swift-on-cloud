...
#==================================================================================
    ...
    if len(args) >=  2 :
        request_id = args[1]
        if request_id in [ r.id for r in requests]:
            print "Request is present"
            conn.cancel_spot_instance_requests([request_id], dry_run=configs['AWS_DRY_RUN'])
...
elif args[0] == "tag_and_attach_eip":
    requests = conn.get_all_spot_instance_requests()
    instance_ids = [ i.instance_id for i in requests if i.state == "active" ]

    for request in requests :
        if request.state == "active" :
            instance_id = request.instance_id
            print "instance_id : {0}".format(instance_id)
            instance    = cloud.get_instances(configs, conn, {'instance_id': instance_id })
            instance[0].add_tag('Name', "sworker-{0}".format(instance[0].id))
            instance[0].add_tag('subnet_id', network['WORKER_SUBNET'])
            address = cloud.setup_elastic_ip(conn)
            conn.associate_address(instance_id=instance[0].id, allocation_id=address.allocation_id)
